...
func _compose_config() -> Dictionary:
	...
	# v20.1.0 TXServer per-mod muzzle
	var visual_muzzle: Variant = turret_visual.get("asset_muzzle", null)
	if visual_muzzle is Array and visual_muzzle.size() >= 3:
		cfg["muzzle"] = Vector3(float(visual_muzzle[0]), float(visual_muzzle[1]), float(visual_muzzle[2]))
